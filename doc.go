...
//
// Configuration is explicit. Config records the local rank and backend mode.
// The direct backend also uses the rank-zero coordinator address, the
// rank-by-rank RDMA device matrix, and whether a valid ring topology should be
// preferred. The daemon backend uses Rank, Size, and DaemonSocket; it does not
// require the direct backend coordinator or device matrix. Empty and "auto" use
// the working direct backend today; "daemon" uses the jaccld IPC backend.
// Daemon-backed collectives use the daemon asynchronous work protocol when the
// daemon transport supports collective work.
//
...
// reads the JACCL_RANK, JACCL_SIZE, JACCL_IBV_DEVICES, JACCL_COORDINATOR,
// JACCL_RING, JACCL_BACKEND, and JACCL_DAEMON_SOCKET environment variables.
// The rank, size, coordinator, device, and ring variables have MLX_* fallbacks
// matching MLX JACCL where available; the backend and daemon socket variables
// are Go-specific and do not use MLX fallbacks.
//
...
// len(dst) == g.Size()*len(src). Zero-length collectives still use the group
// operation path, so closed groups and canceled contexts are reported.
//
